...
full <- full_join(happines, world_coordinates,
          ...
          by="region")
full$`Happiness gap` <- factor(round(full$`Happiness gap`,0))

full <- full |> 
  mutate(`Happiness gap` = recode(`Happiness gap`,
                                  "2" = "1.5 - 2.49",
                                  "3" = "2.5 - 3.49",
                                  "4" = "3.5 - 4.49",
                                  "5" = "4.5 - 5.49",
...
p1 <- ggplot(full, aes(x = long,
  ...
  labs(title = "Inequality of happiness",
       ...
       caption = "Data Source: World Happiness Report 2023\n#30DayMapChallenge\nThiyanga S. Talagala") + theme_minimal()
p1
